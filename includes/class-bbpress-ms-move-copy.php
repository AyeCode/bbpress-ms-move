...
class Bbpress_Ms_Move_Copy {
	...
	public function ajax_handler() {
		...
		if ( isset( $_POST['bbpc_action'] ) && $_POST['bbpc_action'] ) {
			...
			if ( $bbpc_action == 'copy_settings' ) {
				...
				$both_have_bbp = true;
				if ( ! get_blog_option( $bbpc_from, '_bbp_db_version' ) ) {
					$both_have_bbp = false;
				}
				if ( ! get_blog_option( $bbpc_to, '_bbp_db_version' ) ) {
					$both_have_bbp = false;
				}
	...
	public function copy_settings( $bbpc_from, $bbpc_to ) {
		...
		foreach ( $settings as $setting ) {
			if ( $value = get_blog_option( $bbpc_from, $setting ) ) {
				update_blog_option( $bbpc_to, $setting, $value );
			}
		}
	...
	public function move_posts( $bbpc_from, $bbpc_to, $limit = 100, $offset = 0, $cpt ) {
		...
		if ( ! empty( $posts ) ) {
			foreach ( $posts as $post ) {
				$this->move_post( $post, $bbpc_from, $bbpc_to );
			}
		}
	...
	public function move_post( $post, $bbpc_from, $bbpc_to ) {
	...
	public function move_term_relationships( $bbpc_from, $bbpc_to, $limit = 100, $offset = 0, $tax ) {
		...
		if ( ! empty( $terms ) ) {
			foreach ( $terms as $term ) {
				$this->move_term_relationship( $term, $bbpc_to );
			}
		}
	...
	public function move_term_relationship( $term, $bbpc_to ) {
	...
	public function move_post_attachments( $bbpc_from, $bbpc_to, $limit = 100, $offset = 0, $cpt ) {
		...
		if ( ! empty( $posts ) ) {
			foreach ( $posts as $post ) {
				$this->move_post_attachment( $post, $bbpc_from, $bbpc_to );
			}
		}
	...
	public function move_post_attachment( $post, $bbpc_from, $bbpc_to ) {
	...
	public function remove_posts( $bbpc_from, $limit = 100, $offset = 0, $cpt ) {
		...
		if ( ! empty( $posts ) ) {
			foreach ( $posts as $post ) {
				$this->remove_post( $post, $bbpc_from );
			}
		}
	...
	public function remove_post( $post, $bbpc_from ) {
	...
	public function remove_term_relationships( $bbpc_from, $limit = 100, $offset = 0, $tax ) {
		...
		if ( ! empty( $terms ) ) {
			foreach ( $terms as $term ) {
				$this->remove_term_relationship( $term, $bbpc_from );
			}
		}
	...
	public function remove_term_relationship( $term, $bbpc_from ) {
